...
func (s *Storage) SaveUser(ctx context.Context, email string, passwdHash []byte) (int64, error) {
	...
	if err != nil {
		...
		if errors.As(err, &sqliteErr) && sqliteErr.ExtendedCode == sqlite3.ErrConstraintUnique {
			return 0, fmt.Errorf("%s: %w", op, storage.ErrExistUser)
		}
		return 0, fmt.Errorf("%s: %w", op, err)
	}
...
func (s *Storage) IsAdmin(ctx context.Context, userId int64) (bool, error) {
	...
	stmt, err := s.db.PrepareContext(ctx, "SELECT id_admin FROM users WHERE id=?")
	if err != nil {
